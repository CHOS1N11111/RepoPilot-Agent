...
- 在一次决策中并发执行 2 到 4 个互相独立的只读工具，同时保证结果顺序确定，并按实际工具数核算预算。
...
RepoPilot 会按目录作用域读取仓库内部的 `AGENTS.md`：根目录规则全局生效，子目录规则只影响对应目录树。Summary 会显示实际应用的路径、作用域、优先级、哈希和脱敏后的有限内容；这些规则不能绕过审批、沙箱、命令白名单或密钥保护。
