...
`parallel_read` accepts an ordered list of 2 to 4 unique actions chosen only from search, file read, Repository Map inspection, Git status, and diff inspection. Use it only when every input is known before execution and no member depends on another member's result. One batch consumes one Agent step but each member consumes one tool call. Results, selected paths, persisted observations, and prompts retain request order even when members finish in another order. Successful members remain usable when another member fails; an all-failed batch fails as a whole. RepoPilot keeps patch proposals, writes, commands, validation, questions, and finish decisions serialized.
...
- `agent_steps`: non-writing iterative decisions with rationale, expected evidence, state update, observation, selected paths, per-step tool-call count, and finish reason.
...
- `sandbox_inspection`: preserve and inspect the sandbox after an interrupted write, validation, diagnosis, or re-planning phase; uncommitted changes are allowed because they may be recovery evidence.
...
`Check Readiness` runs a read-only recovery preflight and displays each check as passed, failed, warning, or not required. It compares the Resume Plan and latest execution checkpoint, verifies the required source or sandbox, applies checkpoint-specific Git cleanliness rules, checks the saved sandbox HEAD, confirms persisted proposal sessions, and shows the Runtime action classification. Legacy tasks without Runtime events receive a phase-level recovery warning instead of being permanently blocked. The check does not call an LLM, run validation, write files, or change task status.

For iterative runs, the Runtime Resume panel also shows the Working State iteration, number of replayed read-only observations, next recovery step, and any exact pending action. Recovery follows these rules:

- A completed observation after the last valid snapshot is folded into Working State instead of rerunning the phase.
- A decided but not-started action continues under its original policy and approval boundary.
...
Clicking Resume refreshes that report before opening a confirmation that names the checkpoint or exact ambiguous action. The request includes the checkpoint and, when required, the exact action id and payload-bound recovery token, preventing a stale browser from approving a different recovery path. The server reruns readiness after confirmation and then validates request-scoped LLM configuration before changing the task state. If any step fails, the task remains `interrupted`, `paused`, `failed`, or `cancelled` with the sandbox preserved. A successful resume records its UTC time, checkpoint, and cumulative resume count, then continues the existing Runtime run id from the next decision. API keys are not part of the readiness report or persisted recovery summary.
