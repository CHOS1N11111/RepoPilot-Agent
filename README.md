...
- Run-scoped virtual patch proposals with exact hunks, SHA-256 revision guards, syntax checks, cumulative diff review, and no repository writes.
...
| Agent working state    | Persists bounded focus, plans, acceptance evidence, virtual-edit metadata, and lifecycle snapshots.    |
| Agent context manager  | Rebuilds prioritized, section-budgeted, redacted context before every iterative decision.             |
| Iterative agent        | Uses non-writing tools for bounded exploration, virtual patch revision, and diff inspection.          |
| Virtual patch overlay  | Applies exact hunks in memory, rechecks real file hashes, and exposes cumulative proposed diffs.       |
...
src/repopilot_agent/
  ...
  agent_loop.py         LLM analysis and virtual-proposal adapter for the typed runtime
  ...
  runtime/
    ...
    tools.py            typed repository, Git, virtual proposal, edit, and validation tools
    virtual_patch.py    run-scoped in-memory patch revisions and cumulative diff inspection
...
Enable non-writing iterative agent mode when you want Codex-like multi-step exploration and virtual edit preparation before the main planning and proposal calls:

```bash
python repopilot.py run --repo . --task "fix parser behavior" --use-llm --iterative-agent --agent-max-steps 6
```

In this mode, the LLM can choose bounded non-writing actions: `search_files`, `read_file`, `inspect_repository_map`, `inspect_git_status`, `inspect_diff`, `propose_patch`, and `inspect_proposed_diff`. Every call must return the strict `AgentDecision` v2 envelope: a rationale, exactly one action with action-specific arguments, expected evidence, a bounded state update, a finish reason, and a user-question field. State updates can upsert implementation-plan items and acceptance criteria. Unknown fields, unknown arguments, unsafe paths, invalid types, oversized patch hunks, missing evidence, and invalid finish, question, plan, or acceptance semantics are rejected before a tool runs.

These actions run through the same typed runtime used for guarded `apply_patch`, `edit_file`, `run_command`, `validate`, `ask_user`, and `finish` tools. The current Agent policy enables only the non-writing subset. `propose_patch` applies exact old/new text hunks to a run-scoped in-memory overlay; it is distinct from the write-capable `apply_patch` tool. Approved repository edits continue to flow through proposal review and human approval. State updates are controller notes, not automatically accepted validation evidence.

`read_file` observations include a SHA-256 digest of the complete file even when the displayed content is truncated. A first `propose_patch` must cite that real-file digest. A later revision must cite the resulting virtual digest returned by the previous proposal. Every proposal and `inspect_proposed_diff` call rechecks the real baseline, so an external file change becomes a structured `stale_repository` conflict. Re-reading the file and proposing with its new digest resets that file's virtual baseline. Missing or ambiguous hunk matches and invalid Python or JSON output are rejected, while the real file remains byte-for-byte unchanged.

`inspect_proposed_diff` returns one cumulative unified diff from each real baseline to its latest virtual revision and marks those revisions inspected. If a later proposal revises a file, inspection becomes required again. The Agent cannot finish while any virtual edit is conflicted or uninspected. The virtual overlay's complete file contents are intentionally process-local and are not restored after restart; bounded proposal metadata and runtime observations remain available for reporting and History.

Each runtime action has an action id and idempotency key. The controller uses one lifecycle for every non-writing step: decide, record, authorize, execute, observe, update Working State, and either repeat or stop. RepoPilot persists ordered `decision_recorded`, `action_authorized`, `action_started`, `action_completed`, conflict, approval, input, recovery, replay, and `run_stopped` events. Denied and approval-required actions never receive `action_authorized`. A completed action is not executed again when retried with the same payload. If RepoPilot finds an unfinished reservation after a restart, it reports `recovery_required` instead of blindly repeating a possible file write or command.

Working State v4 keeps the Agent's plan and acceptance criteria mutable but evidence-bound and adds bounded virtual-edit metadata: path, real baseline hash, current virtual hash, revision, cumulative hunk count, conflict status, and inspection state. A plan item may become `completed`, and a criterion may become `passed`, only when its update cites a previously completed evidence-producing action id. Search candidates, model findings, proposed edits, expected future output, user questions, and `finish` itself are not completion evidence. Existing required criteria cannot be downgraded to optional. If the model chooses `finish` while plan, acceptance, or proposal-review blockers remain, RepoPilot records `finish_blocked`, returns the blockers as the observation, and continues within the existing step budget without authorizing finish.

Workflow JSON includes `agent_run_id`, `agent_events`, the latest `agent_state`, `agent_stop_reason`, `agent_pending_question`, `agent_completion_ready`, `agent_completion_blockers`, `agent_proposed_edits`, and `agent_proposed_diff`. Working State v4 records the objective, current focus, bounded findings, bounded open questions, implementation plan, acceptance state, virtual-edit metadata, expected evidence, lifecycle fields, selected paths, eight recent bounded observation summaries with action ids, stop reason, and update time. Initial, per-step, and terminal snapshots are stored as `working_state_updated` runtime events, so reopening the SQLite event stream can recover the latest valid snapshot. Version 1 through 3 snapshots remain readable with empty defaults for later fields. Complete file bodies, complete command output, API keys, and provider endpoints are not copied into these snapshots.

The CLI, Web Summary, and saved History views show plan progress, acceptance evidence, virtual proposal revisions and cumulative diff, completion readiness and blockers, the explicit stop result, pending question when present, and the ordered runtime timeline. Agent Steps expose plan and acceptance updates alongside rationale, action input, expected evidence, observations, selected paths, finish reason, and user question. Each iterative prompt receives the compact state and observation action ids needed for evidence references, and the completed state is handed to the downstream planner so its final implementation plan can incorporate the Agent's evidence-backed revisions. The core controller can opt into `ask_user` and stops with `input_required`; the CLI/Web workflow keeps this action disabled until a durable answer-and-resume path is available. The LLM controller remains non-writing and all real file changes still require proposal review and human approval.
...
- Iterative Agent finish decisions are blocked until every plan item is complete and every required Agent acceptance criterion cites a completed evidence-producing observation.
