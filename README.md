...
In this mode, the LLM can choose bounded read-only actions such as `search_files`, `read_file`, `inspect_repository_map`, `inspect_git_status`, and `inspect_diff`. Every call must return the strict `AgentDecision` v2 envelope: a rationale, exactly one action with action-specific arguments, expected evidence, a bounded state update, a finish reason, and a user-question field. State updates can upsert implementation-plan items and acceptance criteria. Unknown fields, unknown arguments, unsafe paths, invalid types, missing evidence, and invalid finish, question, plan, or acceptance semantics are rejected before a tool runs.
...
Working State v3 makes the Agent's plan and acceptance criteria mutable but evidence-bound. A plan item may become `completed`, and a criterion may become `passed`, only when its update cites a previously completed evidence-producing action id. Search candidates, model findings, expected future output, user questions, and `finish` itself are not completion evidence. Existing required criteria cannot be downgraded to optional. If the model chooses `finish` while plan or acceptance blockers remain, RepoPilot records `finish_blocked`, returns the blockers as the observation, and continues within the existing step budget without authorizing finish.

Workflow JSON includes `agent_run_id`, `agent_events`, the latest `agent_state`, `agent_stop_reason`, `agent_pending_question`, `agent_completion_ready`, and `agent_completion_blockers`. Working State v3 records the objective, current focus, bounded findings, bounded open questions, implementation plan, acceptance state, expected evidence, lifecycle fields, selected paths, eight most recent bounded observation summaries with action ids, stop reason, and update time. Initial, per-step, and terminal snapshots are stored as `working_state_updated` runtime events, so reopening the SQLite event stream can recover the latest valid snapshot. Version 1 and 2 snapshots remain readable with empty defaults for later fields. Complete file bodies, complete command output, API keys, and provider endpoints are not copied into these snapshots.

The CLI, Web Summary, and saved History views show plan progress, acceptance evidence, completion readiness and blockers, the explicit stop result, pending question when present, and the ordered runtime timeline. Agent Steps expose plan and acceptance updates alongside rationale, action input, expected evidence, observations, selected paths, finish reason, and user question. Each iterative prompt receives the compact state and observation action ids needed for evidence references, and the completed state is handed to the downstream planner so its final implementation plan can incorporate the Agent's evidence-backed revisions. The core controller can opt into `ask_user` and stops with `input_required`; the CLI/Web workflow keeps this action disabled until a durable answer-and-resume path is available. The LLM controller remains read-only and all file changes still require proposal review and human approval.
