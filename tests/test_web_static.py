...
class WebApprovalUiContractTests(unittest.TestCase):
    ...
    def test_summary_exposes_typed_runtime_events(self) -> None:
        ...
        self.assertIn("step.tool_call_count", self.app_js)
        self.assertIn("read tools", self.app_js)
        self.assertIn("event.payload?.tool_call_cost", self.app_js)
    ...
    def test_autonomous_repair_controls_and_progress_are_visible(self) -> None:
        ...
        self.assertIn("same-controller repair", self.app_js)
        self.assertIn("Baseline validation", self.app_js)
        self.assertIn("...(taskRun.result || {})", self.app_js)
